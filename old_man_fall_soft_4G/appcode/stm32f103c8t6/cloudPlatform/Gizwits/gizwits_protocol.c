...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(uint8_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagTEMP_MAX)
    {
        info->event[info->num] = EVENT_TEMP_MAX;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valueTEMP_MAX,(uint8_t *)&issuedData[byteFieldOffset],TEMP_MAX_LEN);
        byteFieldOffset += TEMP_MAX_LEN;
        dataPoints->valueTEMP_MAX = gizX2Y(TEMP_MAX_RATIO,  TEMP_MAX_ADDITION, elongateP0FormTmp.devStatus.valueTEMP_MAX); 
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagHR_MAX)
    {
        info->event[info->num] = EVENT_HR_MAX;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valueHR_MAX,(uint8_t *)&issuedData[byteFieldOffset],HR_MAX_LEN);
        byteFieldOffset += HR_MAX_LEN;
        dataPoints->valueHR_MAX = gizX2Y(HR_MAX_RATIO,  HR_MAX_ADDITION, elongateP0FormTmp.devStatus.valueHR_MAX); 
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagHR_MIN)
    {
        info->event[info->num] = EVENT_HR_MIN;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valueHR_MIN,(uint8_t *)&issuedData[byteFieldOffset],HR_MIN_LEN);
        byteFieldOffset += HR_MIN_LEN;
        dataPoints->valueHR_MIN = gizX2Y(HR_MIN_RATIO,  HR_MIN_ADDITION, elongateP0FormTmp.devStatus.valueHR_MIN); 
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagSQO2_MIN)
    {
        info->event[info->num] = EVENT_SQO2_MIN;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valueSQO2_MIN,(uint8_t *)&issuedData[byteFieldOffset],SQO2_MIN_LEN);
        byteFieldOffset += SQO2_MIN_LEN;
        dataPoints->valueSQO2_MIN = gizX2Y(SQO2_MIN_RATIO,  SQO2_MIN_ADDITION, elongateP0FormTmp.devStatus.valueSQO2_MIN); 
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagPHONE)
    {
        info->event[info->num] = EVENT_PHONE;
        info->num++;
        gizMemcpy((uint8_t *)dataPoints->valuePHONE,(uint8_t *)&issuedData[byteFieldOffset],12);
        byteFieldOffset += PHONE_LEN;
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuefallSta != cur->valuefallSta)
    {
        GIZWITS_LOG("valuefallSta Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagfallSta = 1;
        ret = 1;
    }
    if(last->valuexinlvSta != cur->valuexinlvSta)
    {
        GIZWITS_LOG("valuexinlvSta Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagxinlvSta = 1;
        ret = 1;
    }
    if(last->valuetempSta != cur->valuetempSta)
    {
        GIZWITS_LOG("valuetempSta Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagtempSta = 1;
        ret = 1;
    }
    if(last->valuexueyangSta != cur->valuexueyangSta)
    {
        GIZWITS_LOG("valuexueyangSta Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagxueyangSta = 1;
        ret = 1;
    }
    if(last->valueTEMP_MAX != cur->valueTEMP_MAX)
    {
        GIZWITS_LOG("valueTEMP_MAX Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagTEMP_MAX = 1;
        ret = 1;
    }
    if(last->valueHR_MAX != cur->valueHR_MAX)
    {
        GIZWITS_LOG("valueHR_MAX Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagHR_MAX = 1;
        ret = 1;
    }
    if(last->valueHR_MIN != cur->valueHR_MIN)
    {
        GIZWITS_LOG("valueHR_MIN Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagHR_MIN = 1;
        ret = 1;
    }
    if(last->valueSQO2_MIN != cur->valueSQO2_MIN)
    {
        GIZWITS_LOG("valueSQO2_MIN Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagSQO2_MIN = 1;
        ret = 1;
    }
    if(0 != memcmp((uint8_t *)&last->valuePHONE,(uint8_t *)&cur->valuePHONE,sizeof(last->valuePHONE)))
    {
        GIZWITS_LOG("valuePHONE Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagPHONE = 1;
        ret = 1;
    }

    if(last->valuexintiao != cur->valuexintiao)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuexintiao Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagxintiao = 1;
            ret = 1;
        }
    }
    if(last->valuexueyang != cur->valuexueyang)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuexueyang Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagxueyang = 1;
            ret = 1;
        }
    }
    if(last->valuetiwen != cur->valuetiwen)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuetiwen Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagtiwen = 1;
            ret = 1;
        }
    }
    if(last->valuelat != cur->valuelat)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuelat Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flaglat = 1;
            ret = 1;
        }
    }
    if(last->valuelon != cur->valuelon)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuelon Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flaglon = 1;
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , uint8_t *outData,uint32_t *outDataLen)
{
    ...
    /*** Fill the bit ***/
    if(gizwitsProtocol.waitReportDatapointFlag.flagfallSta)
    {
        gizVarlenCompressValue(bitFieldOffset,fallSta_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valuefallSta);
        bitFieldOffset += fallSta_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagxinlvSta)
    {
        gizVarlenCompressValue(bitFieldOffset,xinlvSta_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valuexinlvSta);
        bitFieldOffset += xinlvSta_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagtempSta)
    {
        gizVarlenCompressValue(bitFieldOffset,tempSta_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valuetempSta);
        bitFieldOffset += tempSta_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagxueyangSta)
    {
        gizVarlenCompressValue(bitFieldOffset,xueyangSta_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valuexueyangSta);
        bitFieldOffset += xueyangSta_LEN;
    }
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagxintiao)
    {
        devStatusTmp.valuexintiao = gizY2X(xintiao_RATIO,  xintiao_ADDITION, dataPoints->valuexintiao); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuexintiao,xintiao_LEN);
        byteFieldOffset += xintiao_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagxueyang)
    {
        devStatusTmp.valuexueyang = gizY2X(xueyang_RATIO,  xueyang_ADDITION, dataPoints->valuexueyang); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuexueyang,xueyang_LEN);
        byteFieldOffset += xueyang_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagtiwen)
    {
        devStatusTmp.valuetiwen = gizY2X(tiwen_RATIO,  tiwen_ADDITION, dataPoints->valuetiwen); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuetiwen,tiwen_LEN);
        byteFieldOffset += tiwen_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagTEMP_MAX)
    {
        devStatusTmp.valueTEMP_MAX = gizY2X(TEMP_MAX_RATIO,  TEMP_MAX_ADDITION, dataPoints->valueTEMP_MAX); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueTEMP_MAX,TEMP_MAX_LEN);
        byteFieldOffset += TEMP_MAX_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagHR_MAX)
    {
        devStatusTmp.valueHR_MAX = gizY2X(HR_MAX_RATIO,  HR_MAX_ADDITION, dataPoints->valueHR_MAX); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueHR_MAX,HR_MAX_LEN);
        byteFieldOffset += HR_MAX_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagHR_MIN)
    {
        devStatusTmp.valueHR_MIN = gizY2X(HR_MIN_RATIO,  HR_MIN_ADDITION, dataPoints->valueHR_MIN); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueHR_MIN,HR_MIN_LEN);
        byteFieldOffset += HR_MIN_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagSQO2_MIN)
    {
        devStatusTmp.valueSQO2_MIN = gizY2X(SQO2_MIN_RATIO,  SQO2_MIN_ADDITION, dataPoints->valueSQO2_MIN); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueSQO2_MIN,SQO2_MIN_LEN);
        byteFieldOffset += SQO2_MIN_LEN;
    }


    if(gizwitsProtocol.waitReportDatapointFlag.flaglat)
    {
        devStatusTmp.valuelat = exchangeWord(gizY2XFloat(lat_RATIO,  lat_ADDITION, dataPoints->valuelat)); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuelat,lat_LEN);
        byteFieldOffset += lat_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flaglon)
    {
        devStatusTmp.valuelon = exchangeWord(gizY2XFloat(lon_RATIO,  lon_ADDITION, dataPoints->valuelon)); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuelon,lon_LEN);
        byteFieldOffset += lon_LEN;
    }


    if(gizwitsProtocol.waitReportDatapointFlag.flagPHONE)
    {
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)dataPoints->valuePHONE,sizeof(dataPoints->valuePHONE));
        byteFieldOffset += sizeof(dataPoints->valuePHONE);
    }
